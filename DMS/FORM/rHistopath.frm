...
Begin VB.Form rHistopathology 
   BackColor       =   &H00C0B4A9&
   Caption         =   "Lab Report Format [HISTOPATHOLOGY]"
   ClientHeight    =   8595
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11370
   DrawWidth       =   2
   Icon            =   "rHistopath.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ...
   End
End
Attribute VB_Name = "rHistopathology"
...
'===for show data in Datagrid1=============
    ...
    Adodc11.RecordSource = "exec Pro_FLUSH_TN 1,'" + txtM_Code + "','" + txtPat_ID + "'"
    Adodc11.Refresh
...
Private Sub Form_KeyPress(KeyAscii As Integer)
    If KeyAscii = 27 Then
    If rHistopathology.DataGrid1.Visible = True Then
        rHistopathology.DataGrid1.Visible = False
    Else
        Unload Me
    End If
    End If
...
'    Adodc8.RecordSource = "exec m_name_select 2,'" + "CYTOPATHOLOGY / HISTOPATHOLOGY" + "'"
    Adodc8.RecordSource = "exec m_name_select 2,'" + "HISTOPATHOLOGY" + "'"
    Adodc8.Refresh
...
'-------------------------------------------------------------
    ...
    Adodc6.RecordSource = "exec Report_All_Select2 1,'" & Trim(txtPat_ID.Text) & "','" + Trim(txtM_Code.Text) + "','" + Trim(txtS_Code) + "'"
    Adodc6.Refresh
...
'++++++++++for show feild18 to txtNote +++++++++++
    ...
    Adodc8.Refresh
    If Adodc8.Recordset.RecordCount > 0 Then
    txtDx.Text = Adodc8.Recordset!Field14
...
On Error Resume Next
    ...
    Set cmd.ActiveConnection = con
    While Temp_Table1.EOF = False
            cmd.CommandText = "exec Report_All_Delete2 2," & Trim(txtPat_ID.Text) & ",'" + Trim(txtM_Code) + "','" + Trim(txtS_Code) + "'"
            cmd.Execute
            Temp_Table1.MoveNext
    Wend
    con.Close
...
Private Sub GetResult() 'ComTest_Title
  ...
  Adodc7.RecordSource = "exec test_result_SELECT5 1,'" + ComTest_Title + "','" + ComTest_Name + "','" + "10H" + "'"
  Adodc7.Refresh

    If Adodc7.Recordset.RecordCount > 0 Then
    txtTest_Result = Adodc7.Recordset!unit
    txtTest_Name = ComTest_Name
    End If
End Sub
  ...
  Adodc10.Refresh
    ...
    If Adodc10.Recordset.RecordCount > 0 Then
       Do Until Adodc10.Recordset.EOF
          ...
          txtSpecimen = Adodc10.Recordset!ref_range
...
ComTest_Name.Clear
  ...
  Adodc7.connectionstring = strcn.Connection
  Adodc7.RecordSource = "exec test_result_select 1,'" + ComTest_Title + "','" + "10H" + "'"
  Adodc7.Refresh
